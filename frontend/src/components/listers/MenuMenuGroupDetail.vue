<template>
    <v-card outlined>
        <v-card-title>
            MenuGroup # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="Parent" v-model="item.parent" :editMode="editMode" @change="change" />
            </div>
            <div>
            ...
            <div>
                <String label="SelectedImg" v-model="item.selectedImg" :editMode="editMode" @change="change" />
            </div>
            <div>
                <String label="DeselectImg" v-model="item.deselectImg" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/menuGroups/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
